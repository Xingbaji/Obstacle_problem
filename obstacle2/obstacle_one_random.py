...
def generate_random(num_linsapce=100):
    input_uniform = np.random.uniform(0,1,num_linsapce)
    input_uniform = np.reshape(input_uniform,[-1,1])
    U_exact_uniform = cal_u_exact(input_uniform)
...
while iter <= num_iters:
    optimizer.zero_grad()
    input_inner,input_boundary,U_exact_inner,U_exact_boundary,g_inner = generate_random(config['num_linspace'])
