...
def generate_uniform(num_linsapce):
    ...
    X2,Y2 = np.meshgrid(X_axis[1:-1],Y_axis[1:-1])
    X_flat = np.reshape(X2,[-1,1]).squeeze()
    Y_flat = np.reshape(Y2,[-1,1]).squeeze()
...
load_path = '/home/sx/PDE_experiments/results/Obstacle_exp3/0126_1223/'
