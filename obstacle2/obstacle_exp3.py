...
def generate_uniform(num_linsapce):
    ...
    r_tmp = np.sqrt(input_uniform[:,0]**2 + input_uniform[:,1]**2)
    cond = (r_tmp>a) * (r_tmp<b)
    input_square = input_uniform[cond]
    if config['visual'] == True:
        plt.plot(input_square[:,0],input_square[:,1],'.');plt.show()

    x = input_square[:,0]
    y = input_square[:,1]
    r = np.sqrt(x**2+y**2)
    real_part1 = 1/2*np.sqrt( ( (x**2 - y**2)/(r**2) )**2 + 1/4 * ( r**2/(a**2) - a**2/(r**2) )**2)
    real_part2 = 1/4*( (x**2 - y**2)/(r**2)) * ( (r**2/(a**2) + a**2/(r**2) )**2 )
    real = np.sqrt(real_part1 + real_part2) * np.sign(x)
    imag = np.sqrt(real_part1 - real_part2)*np.sign(y)
    v = real.copy()
